...
快速配置使用一个连续面板，不拆分为子卡片。为保证 STEP 01 的主任务仍然是“解析招标文件并识别多标段”，配置默认折叠：

- 默认只显示一条紧凑摘要栏，包含 `快速配置`、当前已生效的篇幅/表格/图片摘要，以及图标按钮 `展开设置`。
- 点击 `展开设置` 后显示完整的四行连续配置；点击 `收起设置` 后恢复为摘要栏，不保留大块空白。
- 折叠/展开只属于 Renderer 的轻量 UI 偏好，使用 `localStorage` 保存，不新增 SQLite 字段、不影响业务配置值；默认值为折叠。
- 已有配置仍然在摘要栏中反映，避免用户误以为没有配置；若存在自定义篇幅或未设置状态，摘要显示 `自定义` / `未设置`。
- 配置面板即使展开，也不能推高整个窗口到依赖 `body` 滚动；页面内部继续按当前工作台规则滚动。
...
- `client/src/features/technical-plan/pages/DocumentAnalysisPage.tsx`
  - 重排 STEP 01 DOM。
  - 将快速配置从卡片网格改成连续四行。
  - 增加默认折叠的配置摘要栏，并使用 `localStorage` 保存展开/折叠偏好。
...
- `client/src/features/technical-plan/services/workflowLayout.test.ts`
  - 增加页面结构约束测试，防止快速配置回退为卡片网格，并覆盖默认折叠摘要、七档映射、默认/显式 `false` 处理、完整正文配置保留和任务状态禁用约束。
...
13. 首次进入 STEP 01 确认配置默认折叠且正文区域前移；展开、收起后刷新页面，确认只恢复 UI 展开偏好，不改变已保存业务配置。
