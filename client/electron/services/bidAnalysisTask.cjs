...
4. 表格左侧合并单元格表示的评分大项、右侧每条独立评分行、评分标准正文、分值和来源位置必须分别保留，并保持原始顺序。
5. 不得提前合并意思相近的评分行，不得把多条评分标准概括成摘要；“项目实施过程中”“服务需求中”等范围限定词必须保留在完整评分原文中。
6. 对每条评分行识别其中明确要求投标人响应的内容，例如“政策背景”“项目技术要求理解”“建设目标”“技术路线”，原文未列出时不要编造。
