...
3. 若存在层级评分项，按原文编号和直接上级字段体现层级关系，不使用缩进替代结构字段。
4. 单位尽量统一为“分”或“%”，必要时注明原文单位。
5. 直接上级类型只能填写“评分维度/汇总容器”“业务分组”或“无”：仅分类、汇总分值且本身不要求投标人展开响应内容的父级是“评分维度/汇总容器”；通过合并单元格、rowspan、连续编号中的中间层级或独立父级行明确统领具体评分项的业务标题是“业务分组”；不能由原文结构可靠证明时填写“无”。
