...
【层级依据类型】：<合并单元格、编号层级、独立父级行、汇总行或无>
【层级依据说明】：<rowspan 数值、父子编号或对应行位置等原文事实；没有可靠依据则写“无”>
【权重/分值】：<具体分值或占比>
【评分标准】：<完整评分原文，不概括、不合并>
【评分行】：<按原顺序逐条列出独立评分行>
【明确响应内容】：<逐项列出评分行中要求响应的具体内容>
【数据来源】：<章节、条款、页码或表格位置>

处理规则：
1. 若没有明确“技术评分表”，根据上下文判断技术评分相关内容。
2. 若评分项以表格形式呈现，保留左侧合并单元格表示的评分大项和右侧每条独立评分行，并标注“[表格数据]”。
3. 若存在层级评分项，按原文编号和直接上级字段体现层级关系，不使用缩进替代结构字段；保持原始顺序和“项目实施过程中”等范围限定词，不得提前合并近义评分行或把多条评分标准概括成摘要。
4. 识别每条评分行中的明确响应内容；原文未明确列举时写“未明确列举”，不得编造。
5. 单位尽量统一为“分”或“%”，必要时注明原文单位。
6. 直接上级类型只能填写“评分维度/汇总容器”“业务分组”或“无”：仅分类、汇总分值且本身不要求投标人展开响应内容的父级是“评分维度/汇总容器”；通过合并单元格、rowspan、连续编号中的中间层级或独立父级行明确统领具体评分项的业务标题是“业务分组”；不能由原文结构可靠证明时填写“无”。
7. 层级依据类型只能填写“合并单元格”“编号层级”“独立父级行”“汇总行”或“无”，层级依据说明记录可核验的 rowspan 数值、父子编号或对应行位置；不得把内容语义、相邻关系或共同主题写成结构依据，不得根据父级名称中的字样判断类型。
8. 评分项名称必须逐字复制评分表中对应的评审因素名称，只允许删除序号、换行、末尾单独标注的分值，以及标题末尾仅表示评分方式的“（客观分）”“（主观分）”；不得删除“项目”“本项目”“服务”“保障”等原文词语，不得从评分标准正文归纳或重写评分项名称。例如，“项目实施方案”不得改写为“实施方案”。
