...
const index: KnowledgeBaseIndex = {
  ...
  documents: [
    document('doc-a-pending', 'folder-a-1', 'pending'),
    document('doc-a-success-1', 'folder-a-1', 'success'),
    document('doc-b-success', 'folder-b-1', 'success'),
    document('doc-a-error', 'folder-a-1', 'error'),
    document('doc-a-success-2', 'folder-a-1', 'success'),
  ],
};
...
test('替换当前目录的文档关联，保留其他目录、移除旧状态文档并去重', () => {
  assert.deepEqual(
    mergeFolderDocumentSelection(
      ...
      ['doc-a-success-2', 'doc-a-success-2', 'doc-a-pending', 'doc-a-success-1'],
    ),
    ['doc-b-success', 'doc-a-success-2', 'doc-a-success-1'],
  );
});

test('未知目录不写入新选择并返回空数组', () => {
  assert.deepEqual(
    mergeFolderDocumentSelection(index, 'folder-unknown', ['doc-b-success'], ['doc-a-success-1']),
    [],
  );
});
